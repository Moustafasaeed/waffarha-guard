...
export default function App(){
  ...
  // ── Import handlers ──
  const doImportPT=()=>{const en=ptRows.map(enrichPayTabsRow);const fraud=detectFraud(en,"CC Details"),high=detectHighAmounts(en),fake=detectFakeDomain(en);setPtRaw(ptRows);setPtEnriched(en);setPtFraud(fraud);setPtHighAmt(high);setPtFakeDom(fake);setPtShowRaw(false);setPtStep("done");addAuditLog({user:currentUser.username,action:"Import",platform:"PayTabs",records:ptRows.length,details:`High: ${fraud.filter(r=>r.risk==="High").length} · Mid: ${fraud.filter(r=>r.risk==="Mid").length} · High Amt: ${high.length} · Fake Domain: ${fake.length}`});};
  const doImportPM=()=>{const wEn=pmRows.map(enrichPaymobRow);const bRows=filterBNPLRows(pmRows);const bEn=bRows.map(enrichBNPLRow);const wFraud=detectFraud(wEn,"wallets"),wHigh=detectHighAmounts(wEn),wFake=detectFakeDomain(wEn),wAbusers=detectWalletAbusers(wEn);const bFraud=detectBNPLFraud(bEn),bHigh=detectHighAmounts(bEn);setPmRaw(pmRows);setPmWEnriched(wEn);setPmWFraud(wFraud);setPmWHighAmt(wHigh);setPmWFakeDom(wFake);setPmWalletAbusers(wAbusers);setPmBEnriched(bEn);setPmBFraud(bFraud);setPmBHighAmt(bHigh);setPmWShowRaw(false);setPmBShowRaw(false);setPmSubTab("wallets");setPmStep("done");addAuditLog({user:currentUser.username,action:"Import",platform:"PayMob",records:pmRows.length,details:`W High: ${wFraud.filter(r=>r.risk==="High").length} Mid: ${wFraud.filter(r=>r.risk==="Mid").length} Abusers: ${wAbusers.length} | BNPL: Susp: ${bFraud.length} HighAmt: ${bHigh.length}`});};
  const doImportAdmin=()=>{const filtered=adminRows.filter(r=>(getCol(r)("User Name")||"").toLowerCase().trim()!=="integration");const en=filtered.map(enrichAdminRow);const payM=detectAdminPayMethods(en),susp=detectAdminSuspected(en),high=detectAdminHighAmt(en),fake=detectAdminFakeDomain(en),recharge=detectRechargeAbusers(en);setAdminSheetRaw(filtered);setAdminEnriched(en);setAdminPayMethods(payM);setAdminSuspected(susp);setAdminHighAmt(high);setAdminFakeDom(fake);setAdminRechargeAbusers(recharge);setAdminShowRaw(false);setAdminStep("done");addAuditLog({user:currentUser.username,action:"Import",platform:"Admin",records:filtered.length,details:`PayMethod: ${payM.length} · Suspected: ${susp.length} · High Amt: ${high.length} · Fake: ${fake.length} · Recharge: ${recharge.length}`});};
  const doImportFawry=()=>{const en=fawryRows.map(enrichFawryRow);const high=detectFawryHighAmt(en),susp=detectFawrySuspected(en),fake=detectFawryFakeDomain(en);setFawryRaw(fawryRows);setFawryEnriched(en);setFawryHighAmt(high);setFawrySuspected(susp);setFawryFakeDom(fake);setFawryShowRaw(false);setFawryStep("done");addAuditLog({user:currentUser.username,action:"Import",platform:"Fawry",records:fawryRows.length,details:`Suspected: ${susp.length} · High Amt: ${high.length} · Fake Domain: ${fake.length}`});};
